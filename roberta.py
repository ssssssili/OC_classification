...
class BertClassifier(nn.Module):
    def __init__(self, dropout=0.5):
        ...
        self.linear = nn.Linear(50265, 389)#*******此处的389改为类的数量即可，一定记得需要改变如果切换数据集的话!!!!!!********
        self.relu = nn.ReLU()
...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    plt.title("Condition 1", fontdict={'size': 20})
...
def evaluate(model, test_data):
    ...
    np.savetxt('pred_labels.txt', pred_labels, fmt = '%f') 
#去除一个warning的提示
...
LR = 1e-5
unfreeze_layers = ['layer.11', 'bert.lm_head', 'dropout.', 'linear.', 'relu.']
