...
class BertClassifier(nn.Module):
    def __init__(self, dropout=0.5):
        ...
        self.linear = nn.Linear(768, 3966)#*******此处的389改为类的数量即可，一定记得需要改变如果切换数据集的话!!!!!!********
        self.relu = nn.ReLU()
...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    plt.title("ISCO68 + Original + Bert + Unfreeze 0 layer", fontdict={'size': 20})
    plt.savefig('exp21/21_loss.png')
    ...
    plt.title("ISCO68 + Original + Bert + Unfreeze 0 layer", fontdict={'size': 20})
    plt.savefig('exp21/21_accuracy.png')
...
def evaluate(model, test_data):
    ...
    np.savetxt('exp21/test_labels.txt', test_labels, fmt = '%f')
    np.savetxt('exp21/pred_labels.txt', pred_labels, fmt = '%f') 
...
isco68_feature_ori = data_preprocess.CombineFeature(isco68_prep, column=['bjobnm', 'bjobdes', 'bjobcertain'], withname= False)

isco68_ori = isco68_feature_ori[['feature','label']]
#去除一个warning的提示
