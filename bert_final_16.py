...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    plt.title("ISCO88 + Plus + Bert + Unfreeze all layer", fontdict={'size': 20})
    plt.savefig('exp16/16_loss.png')
    ...
    plt.title("ISCO88 + Plus + Bert + Unfreeze all layer", fontdict={'size': 20})
    plt.savefig('exp16/16_accuracy.png')
...
def evaluate(model, test_data):
    ...
    np.savetxt('exp16/test_labels.txt', test_labels, fmt = '%f')
    np.savetxt('exp16/pred_labels.txt', pred_labels, fmt = '%f') 
