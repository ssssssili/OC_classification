...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    plt.title("ISCO88 + Original + Bert + Unfreeze 1 layer", fontdict={'size': 20})
    plt.savefig('exp10/10_loss.png')
    ...
    plt.title("ISCO88 + Original + Bert + Unfreeze 1 layer", fontdict={'size': 20})
    plt.savefig('exp10/10_accuracy.png')
...
def evaluate(model, test_data):
    ...
    np.savetxt('exp10/test_labels.txt', test_labels, fmt = '%f')
    np.savetxt('exp10/pred_labels.txt', pred_labels, fmt = '%f') 
#去除一个warning的提示
