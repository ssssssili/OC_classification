...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    plt.title("PCS + Original + Bert + Unfreeze 1 layer", fontdict={'size': 20})
    plt.savefig('exp23/23_loss.png')
    ...
    plt.title("PCS + Original + Bert + Unfreeze 1 layer", fontdict={'size': 20})
    plt.savefig('exp23/23_accuracy.png')
...
def evaluate(model, test_data):
    ...
    np.savetxt('exp23/test_labels.txt', test_labels, fmt = '%f')
    np.savetxt('exp23/pred_labels.txt', pred_labels, fmt = '%f')    
...
pcs_feature_ori = data_preprocess.CombineFeature(pcs_prep, column=['numep', 'profession_txt',
                                                                  'secteur_txt'], withname= False)

naf_feature_ori = data_preprocess.CombineFeature(naf_prep, column=['numep', 'profession_txt',
                                                               'secteur_txt'], withname= False)

pcs_ori = pcs_feature_ori[['feature', 'label']]
naf_ori = naf_feature_ori[['feature', 'label']]
