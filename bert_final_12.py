...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    plt.title("ISCO88 + Plus + Bert + Unfreeze 2 layer", fontdict={'size': 20})
    plt.savefig('exp12/12_loss.png')
    ...
    plt.title("ISCO88 + Plus + Bert + Unfreeze 2 layer", fontdict={'size': 20})
    plt.savefig('exp12/12_accuracy.png')
...
def evaluate(model, test_data):
    ...
    np.savetxt('exp12/test_labels.txt', test_labels, fmt = '%f')
    np.savetxt('exp12/pred_labels.txt', pred_labels, fmt = '%f') 
...
LR = 1e-5
unfreeze_layers = ['layer10', 'layer.11', 'bert.pooler', 'dropout.', 'linear.', 'relu.']
