...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    plt.title("NAF + Plus + Bert + Unfreeze 2 layer", fontdict={'size': 20})
    plt.savefig('exp24/24_loss.png')
    ...
    plt.title("NAF + Plus + Bert + Unfreeze 2 layer", fontdict={'size': 20})
    plt.savefig('exp24/24_accuracy.png')
...
def evaluate(model, test_data):
    ...
    np.savetxt('exp24/test_labels.txt', test_labels, fmt = '%f')
    np.savetxt('exp24/pred_labels.txt', pred_labels, fmt = '%f')    
