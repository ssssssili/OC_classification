...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, df):
        self.labels = [labels[label] for label in df['isco88_cg_4']]
...
class BertClassifier(nn.Module):
    def __init__(self, dropout=0.5):
        ...
        self.linear = nn.Linear(768, 389)#*******此处的389改为类的数量即可，一定记得需要改变如果切换数据集的话!!!!!!********
        self.relu = nn.ReLU()
...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    plt.title("ISCO88 + Original + Bert + Unfreeze all layer", fontdict={'size': 20})
    plt.savefig('exp20/20_loss.png')
    ...
    plt.title("ISCO88 + Original + Bert + Unfreeze all layer", fontdict={'size': 20})
    plt.savefig('exp20/20_accuracy.png')
...
def evaluate(model, test_data):
    ...
    np.savetxt('exp20/test_labels.txt', test_labels, fmt = '%f')
    np.savetxt('exp20/pred_labels.txt', pred_labels, fmt = '%f') 
#去除一个warning的提示
...
#读取dataframe
data = pd.read_csv('/home/bme001/20225898/OC_classification/ISCO-88.csv', sep = ',')
#以下代码设置labels为词典
labels = dict()
col = data.iloc[:,1]
arrs = col.values
