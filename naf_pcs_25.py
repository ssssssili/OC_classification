...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    plt.title("PCS + Plus + Bert + Unfreeze 2 layer", fontdict={'size': 20})
    plt.savefig('exp25/25_loss.png')
    ...
    plt.title("PCS + Plus + Bert + Unfreeze 2 layer", fontdict={'size': 20})
    plt.savefig('exp25/25_accuracy.png')
...
def evaluate(model, test_data):
    ...
    np.savetxt('exp25/test_labels.txt', test_labels, fmt = '%f')
    np.savetxt('exp25/pred_labels.txt', pred_labels, fmt = '%f')    
...
LR = 1e-5
unfreeze_layers = ['layer.10','layer.11','bert.pooler', 'dropout.', 'linear.', 'relu.']
